...
del /Q NuoPhpPdo.suo
cmake -G "Visual Studio 10" -D NUODB_ROOT=%NUODB_ROOT% -D NUODB_INCLUDE_DIR=%NUODB_ROOT%/include -D NUODB_LIB_DIR=%NUODB_ROOT%/lib -D NUODB_REMOTE_LIBRARY=%NUODB_ROOT%/lib -D PHP_ROOT=%PHP_INSTALL_DIR% -D PHP_INCLUDE_DIR=%PHP_INSTALL_DIR%/SDK/include .
